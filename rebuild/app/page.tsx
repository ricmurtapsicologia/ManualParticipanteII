...
type BookPage = { number?: number; part?: number | null; partTitle?: string; chapter?: number | null; title: string; paragraphs: string[]; cover?: boolean };
...
export default function Home() {
  ...
  return <div className="shell" data-testid="reader-shell" data-page-count={pages.length} data-wave="8" data-editorial-wave="13">
    ...
    <main className="main"><div className="book"><article className={`page${current.cover ? ' cover' : ''}`} lang="pt-BR" data-testid="book-page">{current.cover ? <div className="coverContent"><div className="coverEyebrow">Corpo de Bombeiros Militar de Minas Gerais</div><h1>{current.title}</h1>{current.paragraphs.map((text, i) => <p key={i}>{text}</p>)}</div> : <><div className="running"><span>{runningLeft}</span><span>{runningRight}</span></div><h2>{current.title}</h2>{current.paragraphs.map((text, i) => <p key={i}>{text}</p>)}<div className="pageno">{currentPageNumber}</div></>}</article></div><div className="status">Use <span className="kbd">←</span> <span className="kbd">→</span> para navegar. <span className="kbd">/</span> abre a busca. Conteúdo canônico.</div></main>
