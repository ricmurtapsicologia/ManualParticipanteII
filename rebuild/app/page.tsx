...
import learningData from '../content/chapter-learning-assets.json';
...
import { ChapterMicrolearning, ChapterResource, ChapterTransfer, type ChapterLearningAsset } from './wave10';
...
type MultimediaArtifact = { schemaVersion: number; resources: MultimediaResource[] };
type QuizArtifact = { schemaVersion: number; chapters: ChapterQuizData[] };
type LearningArtifact = { schemaVersion: number; chapters: ChapterLearningAsset[] };
...
const learningAssets = learningData as LearningArtifact;
...
function MicrolearningCard({ resource }: { resource: MultimediaResource }) {
  ...
  return <section className="pedagogicalBox" data-testid="microlearning-resource" aria-labelledby={labelId} style={{ ['--box-accent' as string]: 'var(--ds2-decide)', ['--box-tint' as string]: '#f7f0f8' }}>
    ...
    <div className="pedagogicalBody">
      <p data-testid="microlearning-prompt" style={{ fontWeight: 550, textAlign: 'left' }}>{resource.prompt}</p>
      <div data-testid="microlearning-choices" role="group" aria-label="Escolha uma resposta" style={{ display: 'grid', gap: 8, marginTop: 12 }}>
        {choices.map(choice => {
          ...
          return <button key={choice.id} type="button" data-testid="microlearning-choice" aria-pressed={active} onClick={() => setSelected(choice.id)} style={{ minHeight: 42, padding: '9px 11px', borderRadius: 9, border: `1px solid ${active ? 'var(--ds2-decide)' : 'var(--ds2-line)'}`, background: active ? '#efe2f2' : '#fff', color: 'var(--ds2-ink)', fontWeight: 500, textAlign: 'left', cursor: 'pointer' }}>{choice.label}</button>;
        })}
      </div>
      {selectedChoice && <div data-testid="microlearning-feedback" aria-live="polite" style={{ marginTop: 12, padding: '10px 11px', borderRadius: 9, background: selectedChoice.correct ? '#edf6f4' : '#fff4ed', border: `1px solid ${selectedChoice.correct ? 'var(--ds2-teal)' : 'var(--ds2-attention)'}` }}>
        <span style={{ fontWeight: 600 }}>{selectedChoice.correct ? 'Correto.' : 'Revise este ponto.'}</span>
...
function renderMultimediaResource(resource: MultimediaResource) {
  ...
  return <figure key={resource.id} className="multimediaFigure atsMacro" data-testid="multimedia-resource" aria-labelledby={labelId} aria-describedby={descriptionId}>
    ...
    <div className="atsFlow" role="list" aria-label="Fases operacionais do Sistema ATS">
      {resource.steps.sort((a, b) => a.order - b.order).map(step => <div key={step.order} className="atsFlowStep" data-testid="multimedia-step" role="listitem"><span className="atsStepNo" aria-hidden="true">{step.order}</span><div><strong>{step.title}</strong><span>{step.detail}</span></div></div>)}
    </div>
...
export default function Home() {
  ...
  const learningAsset = currentChapter ? learningAssets.chapters.find(item => item.chapter === currentChapter.chapter) ?? null : null;
  const isChapterOpening = Boolean(currentChapter && currentChapter.openingPage === currentPageNumber);
  const isChapterEnding = Boolean(currentQuiz && learningAsset);
  ...
  const pageRole = current.cover ? 'cover' : currentChapter ? (isChapterOpening ? 'chapter-opening' : 'chapter-continuation') : currentPart?.openingPage === currentPageNumber ? 'part-opening' : 'standard';
  const isReferencePage = currentPageNumber >= 242 && currentPageNumber <= 244;

  return <div className="shell" data-testid="reader-shell">
    ...
    <main className="main"><div className="book"><div className={`readerSurface${readerTurn.className ? ` ${readerTurn.className}` : ''}`} data-testid="reader-surface" data-turn-direction={readerTurn.direction ?? 'idle'} data-dragging={readerTurn.dragging ? 'true' : 'false'} style={readerTurn.style} tabIndex={0} role="region" aria-label={`Leitor do Manual CATS, página ${currentPageNumber} de ${pages.length}`} aria-describedby="reader-instructions" {...readerTurn.gestureProps}>
      <article className={`page${current.cover ? ' cover' : ''}${pageRole === 'chapter-opening' ? ' chapterOpening' : ''}${pageRole === 'chapter-continuation' ? ' chapterContinuation' : ''}`} lang="pt-BR" data-testid="book-page" data-page-role={pageRole} data-reference-page={isReferencePage ? 'true' : undefined}>{current.cover ? <ApprovedCover /> : <><div className="running"><span>{runningLeft}</span><span>{runningRight}</span></div>{pageRole === 'chapter-continuation' && currentChapter ? <div className="continuationHeading" data-testid="continuation-heading"><span>Capítulo {currentChapter.chapter}</span><span>Continuação</span></div> : <h2 data-testid={pageRole === 'chapter-opening' ? 'chapter-title' : undefined}>{current.title}</h2>}{pageMedia.length > 0 && <div className="multimediaLayer" data-testid="multimedia-layer">{pageMedia.map(renderMultimediaResource)}</div>}{renderSemanticBlocks(current.blocks)}{isChapterOpening && learningAsset && <ChapterMicrolearning asset={learningAsset} />}{isChapterEnding && learningAsset && <><ChapterTransfer asset={learningAsset} /><ChapterResource asset={learningAsset} /></>}{currentQuiz && <ChapterQuiz quiz={currentQuiz} />}<div className="pageno">{currentPageNumber}</div></>}</article>
      <div className="readerAnnouncement" aria-live="polite" aria-atomic="true" data-testid="reader-announcement">Página {currentPageNumber} de {pages.length}</div></div></div><div className="status" id="reader-instructions"><span className="readerGestureHint"><span className="gestureWord">Deslize ou arraste</span> para virar a página, clique nas bordas ou use <span className="kbd">←</span> <span className="kbd">→</span>. <span className="kbd">/</span> abre a busca.</span></div></main>
    ...
    {drawer && <div className="drawer" role="dialog" aria-modal="true" onMouseDown={event => { if (event.target === event.currentTarget) setDrawer(null); }}><aside className="panel"><div className="panelHead"><strong>{drawer === 'toc' ? 'Sumário' : 'Pesquisar'}</strong><button onClick={() => setDrawer(null)}>Fechar</button></div>{drawer === 'toc' ? <div className="toc" data-testid="hierarchical-toc">
