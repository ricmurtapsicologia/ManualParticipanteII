...
function renderPlainBlock(block: SemanticBlock) {
  ...
  if (block.kind === 'external-link' && block.url) return <a key={block.id} className="externalResourceLink" href={block.url} target="_blank" rel="noopener noreferrer" data-kind="external-link">{block.text} <span aria-hidden="true">↗</span></a>;
