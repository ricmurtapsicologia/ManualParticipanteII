...
// Fingerprints específicos das três folhas removidas; a expressão genérica “hierarquia de fontes” pode existir legitimamente no desenvolvimento didático.
const removedAnnex = /COMANDANTE-GERAL DO CBMMG|CORONEL BM JORDANA DE OLIVEIRA FILGUEIRAS DALDEGAN|Precedência para leitura, ensino e atualização doutrinária|Minutas V3\.1 a V3\.4/iu;
...
test('all 34 chapters expose objectives, summary, microlearning, transfer, resource and 5x4 quiz contract',async({page})=>{
  ...
  for(const chapter of enrichment.chapters){
    ...
    expect(quiz.questions,`chapter ${chapter.chapter} quiz`).toHaveLength(5);
    for(const question of quiz.questions){
      expect(question.choices,`${question.id} choices`).toHaveLength(4);
      expect(question.choices.filter(choice=>choice.correct),`${question.id} correct`).toHaveLength(1);
    }
    ...
    await expect(page.locator('[data-kind="objectives"]',),`chapter ${chapter.chapter} objectives`).toHaveCount(1);

    await loadSavedPage(page,chapter.microlearning.pageNumber);
    const micro=page.getByTestId('chapter-microlearning');
    await expect(micro,`chapter ${chapter.chapter} microlearning`).toHaveCount(1);
    ...
    await expect(page.locator('[data-kind="summary"]'),`chapter ${chapter.chapter} summary`).toHaveCount(1);
    const transfer=page.getByTestId('application-transfer');
    await expect(transfer,`chapter ${chapter.chapter} transfer at page ${chapter.endingPage}`).toHaveCount(1);
    ...
    await expect(resource,`chapter ${chapter.chapter} resource at page ${chapter.endingPage}`).toHaveCount(1);
    ...
    await expect(chapterQuiz,`chapter ${chapter.chapter} quiz UI at page ${chapter.endingPage}`).toHaveCount(1);
