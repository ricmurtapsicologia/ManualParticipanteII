...
test('page 1 renders one canonical cover and no extra hero preview', async ({ page }) => {
  for (const width of [320,360,390,430,768,1024,1366,1920]) {
    ...
    const metrics = await page.evaluate(() => {
      const covers=[...document.querySelectorAll<HTMLImageElement>('[data-testid="approved-cover-image"]')];
      const cover=covers[0];
      const heroCount=document.querySelectorAll('[data-testid="canonical-hero"]').length;
      return {viewport:document.documentElement.clientWidth,scroll:document.documentElement.scrollWidth,coverCount:covers.length,heroCount,cover:cover&&{w:cover.clientWidth,h:cover.clientHeight,nw:cover.naturalWidth,nh:cover.naturalHeight}};
    });
    expect(metrics.scroll).toBeLessThanOrEqual(metrics.viewport+1);
    expect(metrics.coverCount).toBe(1);
...
test('320px narrow phone keeps downloads explicit and removes auxiliary overlays', async ({ page }) => {
  ...
  const integrity = await page.evaluate(() => {
    ...
    const downloadLabels=[...document.querySelectorAll<HTMLElement>('.formatDownloadLabel')].map(label=>({text:label.textContent?.trim()??'',display:getComputedStyle(label).display,visibility:getComputedStyle(label).visibility}));
    const debugOverlays=[...document.querySelectorAll<HTMLElement>('[data-debug-overlay],[data-axis-overlay],[data-coordinate-overlay],[data-xy-overlay],[data-debug="true"],.debug-overlay,.debugOverlay,.axis-overlay,.axisOverlay,.coordinate-overlay,.coordinateOverlay,.xy-overlay,.xyOverlay,.debug-axis,.debugAxis,.debug-coordinates,.debugCoordinates')].filter(item=>getComputedStyle(item).display!=='none').length;
    return {viewportWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,header:header&&{left:header.left,right:header.right},paper:paper&&{left:paper.left,right:paper.right},nav:nav&&{left:nav.left,right:nav.right},toolbarButtons:toolbarButtons.map(item=>({width:item.width,height:item.height})),downloadLabels,debugOverlays};
  });
  ...
  expect(integrity.downloadLabels.map(item=>item.text)).toEqual(expect.arrayContaining(['Baixar PDF','Baixar EPUB']));
  expect(integrity.downloadLabels.every(item=>item.display!=='none'&&item.visibility!=='hidden')).toBe(true);
  expect(integrity.debugOverlays).toBe(0);
