...
test('page 54 renders the source-grounded ATS macro infographic without replacing chapter text', async ({ page }) => {
  ...
  await expect(resource).toHaveAttribute('data-media-id','ats-system-macro-p54'); await expect(resource).toHaveAttribute('data-media-src','native://ats-system-macro');
  await expect(resource).toContainText('Sistema ATS • visão macro'); await expect(resource).toContainText('REAVALIAR QUANDO RISCO, AMBIENTE, COMPORTAMENTO, RECURSOS OU RESPOSTA MUDAREM');
  ...
  const paper = page.getByTestId('book-page'); await expect(paper).toContainText('Fases operacionais com avaliação dinâmica transversal.'); await expect(paper).toContainText('1 Acionamento Receber e qualificar dados.');
});
...
test('page 54 native infographic remains inside the paper at 320px', async ({ page }) => {
  ...
  expect(integrity.scrollWidth).toBeLessThanOrEqual(integrity.viewportWidth+1); expect(integrity.paper).toBeTruthy(); expect(integrity.figure).toBeTruthy(); expect(integrity.figure!.left).toBeGreaterThanOrEqual(integrity.paper!.left-1); expect(integrity.figure!.right).toBeLessThanOrEqual(integrity.paper!.right+1); for (const step of integrity.steps){expect(step.left).toBeGreaterThanOrEqual(integrity.figure!.left-1);expect(step.right).toBeLessThanOrEqual(integrity.figure!.right+1);}
});
