...
if (!/data-product="manual-participante-cats"/.test(pageSource)) fail('Stable product marker missing');
if (/data-(?:wave|editorial-wave|design-wave|wave78-status|reader-wave)=/.test(pageSource)) fail('Internal release marker exposed in frontend');
...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*249/.test(health) || !/release:\s*'1\.0\.0'/.test(health)) fail('Health contract mismatch');
if (/\b(?:wave|architecture|corpus|branch|commit)\s*:/i.test(health)) fail('Public health exposes internal metadata');
...
console.log(`VALIDATE_OK pages=249 legacy-prefix=155 recovered-tail=94 source=${SOURCE_SHA256.slice(0, 12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent public-metadata=clean navigation=hierarchical release=1.0.0`);
