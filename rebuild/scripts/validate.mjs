...
if (!Array.isArray(pages) || pages.length !== 155) fail(`Expected 155 pages, got ${pages?.length ?? 'invalid'}`);
...
if (provenance.extractedPages !== 155) fail('Provenance extractedPages mismatch');
if (provenance.recoverableSequentialPages !== 155) fail(`Expected exact recoverable prefix of 155 pages, got ${provenance.recoverableSequentialPages}`);
...
if (!/data-wave="7"/.test(pageSource)) fail('Wave 7 runtime marker missing');
...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*155/.test(health) || !/wave:\s*7/.test(health)) fail('Health contract mismatch');
console.log(`VALIDATE_OK pages=155 sequence=1-155 recoverable=${provenance.recoverableSequentialPages} provenance=${provenance.selectedSha256.slice(0, 12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent health=ok`);
