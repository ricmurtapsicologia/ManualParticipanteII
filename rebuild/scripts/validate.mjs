...
// A fonte canônica preserva 249 páginas; a edição pública final remove 3 folhas de abertura autorizadas.
if (!Array.isArray(pages) || pages.length !== 249) fail(`Expected 249 source pages, got ${pages?.length ?? 'invalid'}`);
pages.forEach((page, index) => {
  if (page.number !== index + 1) fail(`Unexpected number at source page ${index + 1}: ${page.number}`);
  if (!page.title || !Array.isArray(page.paragraphs) || page.paragraphs.length === 0) fail(`Invalid source page ${index + 1}`);
});
if (pages[0].cover !== true) fail('Page 1 must be the cover');
if (!/Eletricidade/i.test([pages[155].title, ...pages[155].paragraphs].join(' '))) fail('Source page 156 does not contain the canonical electricity continuation');
if (!/Checklist final do participante/i.test([pages[248].title, ...pages[248].paragraphs].join(' '))) fail('Source page 249 does not contain the final checklist');
if (!pages[248].paragraphs.join(' ').includes('Tenho um plano para revisar e praticar estas competências após o curso.')) fail('Source canonical ending missing');
...
if (provenance.targetBookPages !== 249) fail(`Unexpected source target size: ${provenance.targetBookPages}`);
...
for (const forbidden of ['data-wave=', 'data-editorial-wave=', 'data-design-wave=', 'data-wave78-status=', 'data-design-system=', 'data-design-subwave=', 'data-semantic-renderer=', 'data-reader-wave=']) if (pageSource.includes(forbidden)) fail(`Backstage runtime marker leaked: ${forbidden}`);
if (!pageSource.includes('enrichmentData') || !pageSource.includes('ChapterMicrolearningCard') || !pageSource.includes('ApplicationTransferCard') || !pageSource.includes('ChapterResourceCard')) fail('Wave10 chapter enrichment contract missing');
...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*246/.test(health) || !/chapters:\s*34/.test(health) || !/wave:\s*10/.test(health) || !/ABNT NBR 6023:2018/.test(health)) fail('Health contract mismatch');
if (!/removedFrontMatterPages:\s*\[2,\s*4,\s*5\]/.test(health)) fail('Health removed-page proof missing');
...
if (deployContract.expected?.wave !== 10 || deployContract.expected?.pages !== 246 || deployContract.expected?.chapters !== 34 || deployContract.expected?.environment !== 'production' || deployContract.expected?.branch !== 'main') fail('Vercel expected-state contract mismatch');
if (JSON.stringify(deployContract.expected?.removedFrontMatterPages) !== JSON.stringify([2,4,5])) fail('Vercel removed-page contract mismatch');
...
console.log(`VALIDATE_OK source-pages=249 public-pages=246 removed=2,4,5 legacy-prefix=155 recovered-tail=94 source=${SOURCE_SHA256.slice(0,12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent health=wave10 backstage=clean navigation=hierarchical`);
