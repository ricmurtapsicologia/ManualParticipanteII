...
if (!Array.isArray(pages) || pages.length !== 150) fail(`Expected 150 pages, got ${pages?.length ?? 'invalid'}`);
...
if (provenance.extractedPages !== 150) fail('Provenance extractedPages mismatch');
if (provenance.recoverableSequentialPages < 150) fail(`Insufficient recoverable pages: ${provenance.recoverableSequentialPages}`);
...
if (!/data-wave="6"/.test(pageSource)) fail('Wave 6 runtime marker missing');
...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*150/.test(health) || !/wave:\s*6/.test(health)) fail('Health contract mismatch');
console.log(`VALIDATE_OK pages=150 sequence=1-150 recoverable=${provenance.recoverableSequentialPages} provenance=${provenance.selectedSha256.slice(0, 12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent health=ok`);
