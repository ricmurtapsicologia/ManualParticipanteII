...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*249/.test(health) || !/release:\s*'2026\.09'/.test(health)) fail('Health contract mismatch');
...
if (deployContract.expected?.pages !== 249 || deployContract.expected?.release !== '2026.09' || deployContract.expected?.environment !== 'production' || deployContract.expected?.branch !== 'main') fail('Vercel expected-state contract mismatch');
...
console.log(`VALIDATE_OK pages=249 legacy-prefix=155 recovered-tail=94 source=${SOURCE_SHA256.slice(0, 12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent health=clean-release vercel-proof=armed navigation=hierarchical`);
