...
if (!/status:\s*'ok'/.test(health) || !/pages:\s*249/.test(health) || !/chapters:\s*34/.test(health) || !/wave:\s*10/.test(health) || !/ABNT NBR 6023:2018/.test(health)) fail('Health contract mismatch');
...
console.log(`VALIDATE_OK pages=249 legacy-prefix=155 recovered-tail=94 source=${SOURCE_SHA256.slice(0,12)} justify=ok tts=Antonio->pt-BR jsdelivr=absent health=wave10 backstage=clean navigation=hierarchical`);
