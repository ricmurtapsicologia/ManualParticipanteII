...
function buildOrderedNavigation(pages) {
  ...
  for (const page of pages) {
    ...
    if (!currentPart || currentPart.part !== partNumber) {
      currentPart = { part: partNumber, title: partNumber === 0 ? 'Elementos iniciais' : page.partTitle || `Parte ${partNumber}`, chapters: [] };
      parts.push(currentPart);
      currentEntry = null;
    }
    const chapterNumber = page.chapter ?? 0;
    const startsNewEntry = !currentEntry || currentEntry.chapter !== chapterNumber || (chapterNumber === 0 && currentEntry.title !== page.title);
    if (startsNewEntry) {
      currentEntry = { chapter: chapterNumber, title: page.title, pageNumbers: [] };
...
function chapterDiagnostics(pages) {
...
export function buildSemanticDocument(sourcePages, sourceSha256) {
  const pages = sourcePages.map((sourcePage, index) => {
    ...
    return {
      ...
      blocks: sourcePage.paragraphs.map((text, sourceIndex) => ({ id: `p${sourcePage.number}-b${sourceIndex + 1}`, kind: classifySemanticBlock(text), sourceIndex, text }))
    };
  });

  const navigation = buildOrderedNavigation(pages);
  const diagnostics = chapterDiagnostics(pages);
  const blockKinds = {};
  const pageRoles = {};
  let blockCount = 0;
  for (const page of pages) {
  ...
  if (diagnostics.boundaryChapterOverlaps.length) {
    editorialAnomalies.push({
      code: 'CHAPTER_METADATA_CROSSES_PART_BOUNDARIES',
      distinctChapterCount: diagnostics.distinctChapterNumbers.length,
      chapterPartPairCount: diagnostics.sourceChapterPairs.length,
      overlaps: diagnostics.boundaryChapterOverlaps,
      action: 'Preserve source metadata in Wave 1; normalize transition-page taxonomy in Wave 2.'
    });
  }
  ...
  return {
    ...
    source: { file: 'pages.json', pageCount: sourcePages.length, sha256: sourceSha256, preservation: 'lossless-title-and-paragraph-text' },
    capabilities: {
    ...
    manifest: {
      ...
      distinctChapterNumberCount: diagnostics.distinctChapterNumbers.length,
      distinctChapterNumbers: diagnostics.distinctChapterNumbers,
      sourceChapterPairCount: diagnostics.sourceChapterPairs.length,
      sourceChapterPairs: diagnostics.sourceChapterPairs,
      boundaryChapterOverlaps: diagnostics.boundaryChapterOverlaps,
