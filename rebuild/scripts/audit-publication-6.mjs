...
const pdfCtas=(html.match(/data-testid="manual-download"/gu)??[]).length;
const epubCtas=(html.match(/data-testid="epub-download"/gu)??[]).length;
add('Reader responde com capa, navegação e CTA único de PDF',home.response.status===200&&/data-testid="approved-cover"/u.test(html)&&/data-page-count="223"/u.test(html)&&pdfCtas===1&&epubCtas===0&&!/Revisão cumulativa|Caso de transferência/iu.test(html));
...
console.log('AUDIT_PUBLICATION_6_PASS controls=6/6 pages=223 chapters=34 runtime=ok pdf=PDF-UA web-security=ok download-cta=pdf-single seo=ok');
