...
page = page.replaceAll(epubAnchor, '');
if (!page.includes(pdfAnchor)) {
  const toolbarAnchor = '<div className="tools"><button onClick={speak}';
  if (!page.includes(toolbarAnchor)) fail('toolbar anchor not found');
  page = page.replace(toolbarAnchor, `<div className="tools">${pdfAnchor}<button onClick={speak}`);
}
if ((page.match(/data-testid="manual-download"/g) ?? []).length !== 1) fail('single PDF CTA contract violated');
if (page.includes('data-testid="epub-download"')) fail('EPUB CTA must not be exposed in the main reader');
...
console.log('RELEASE_FINAL_HOTFIX_OK reader=dynamic download=pdf-single tts=Antonio-only pdf=professional-book cover=strong density<=20pct overlap-guard=on');
