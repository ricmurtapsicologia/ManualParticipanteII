...
const pdfChanged = await patchFile(path.join(scriptsDir, 'generate-accessible-pdf.mjs'), source => {
...
const report = {
  ...
  parity: {
    pdfStaticMicrolearningAndQuiz: pdfChanged || (await readFile(path.join(scriptsDir, 'generate-accessible-pdf.mjs'), 'utf8')).includes('const addStaticQuiz='),
